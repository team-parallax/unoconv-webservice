...
export class Unoconv {
	...
	public static async convert(
		...
		const timeoutPromise = new Promise<Buffer>((resolve, reject) => {
			const maxTime = CMaxAllowedConversionTime
			setTimeout(
				reject,
				maxTime,
		...
		const result = await Promise.race([timeoutPromise, conversionProcessPromise])
		if (result instanceof ConversionTimeoutError) {
			try {
				const pid = result.conversionProcessId
